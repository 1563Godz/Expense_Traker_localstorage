...
// INIT STATE
// ===============================
let activePeriod = 'day';      // day | month | year
let activeType   = 'expense';  // expense | income
let activeSide   = 'expense';  // expense | income
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const monthSelect  = document.getElementById('month-select');
    const yearSelect   = document.getElementById('year-select');
    const tagExpense   = document.getElementById('tag-select-expense');
    const tagIncome    = document.getElementById('tag-select-income');
    ...
    function getFilters() {
        return {
            ...
            tag:    activeType === 'expense' ? (tagExpense ? tagExpense.value : '') : (tagIncome ? tagIncome.value : ''),
            type:   activeType
        };
    }
    ...
    function filterItems(items, filters) {
        return items.filter(item => {
            ...
            if (filters.period === 'month' && filters.month) {
                if (!item.date.includes(filters.month)) return false;
            }
            if (filters.period === 'year' && filters.year) {
                if (!item.date.includes(filters.year)) return false;
            }
            if (filters.tag && filters.tag !== 'all') {
                if (item.tag !== filters.tag) return false;
            }
            return true;
        });
    }
    ...
    function renderSummary() {
        ...
        const items = filters.type === 'expense' ? expenses : incomes;
        ...
        filtered.forEach(item => {
            ...
            // คำนวณแบบง่าย ๆ
            if (d.toDateString() === today.toDateString()) totalDay += amt;
            if (d.getMonth() === today.getMonth() && d.getFullYear() === today.getFullYear()) totalMonth += amt;
            if (d.getFullYear() === today.getFullYear()) totalYear += amt;
        });

        if (document.getElementById('amount-day'))
            document.getElementById('amount-day').textContent   = totalDay.toFixed(2);
        if (document.getElementById('amount-month'))
            document.getElementById('amount-month').textContent = totalMonth.toFixed(2);
        if (document.getElementById('amount-year'))
            document.getElementById('amount-year').textContent  = totalYear.toFixed(2);

        // อัปเดต gain/loss
        const totalIncome = incomes.reduce((s, i) => s + parseFloat(i.amount), 0);
        const totalExpense = expenses.reduce((s, i) => s + parseFloat(i.amount), 0);
        ...
        // render list
        const listEl = filters.type === 'expense' ? expenseList : incomeList;
        if (listEl) {
            listEl.innerHTML = '';
            if (!filtered.length) {
                listEl.innerHTML = `<p class="muted">No ${filters.type} found.</p>`;
            } else {
                filtered.forEach(item => {
                    const div = document.createElement('div');
                    div.className = filters.type === 'expense' ? 'expense-item' : 'income-item';
                    div.innerHTML = `
    ...
    function toggleMainType(type) {
        ...
        if (tagExpense) tagExpense.style.display  = type === 'expense' ? 'block' : 'none';
        if (tagIncome) tagIncome.style.display   = type === 'income' ? 'block' : 'none';
    }
    function toggleSideType(type) {
        sideTabs.forEach(t => t.classList.toggle('active', t.dataset.side === type));
        if (document.getElementById('side-expense-list'))
            document.getElementById('side-expense-list').style.display = type === 'expense' ? 'block' : 'none';
        if (document.getElementById('side-income-list'))
            document.getElementById('side-income-list').style.display  = type === 'income' ? 'block' : 'none';
    }
    ...
    [dateSelect, monthSelect, yearSelect, tagExpense, tagIncome]
        .forEach(el => el && el.addEventListener('change', renderSummary));
